...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...staticRoutes.map(([route, changeFrequency, priority]) => ({ url: `${origin}${route}`, changeFrequency, priority })),
    ...products.map((product) => ({ url: `${origin}/products/${product.slug}`, lastModified: new Date(product.updatedAt), changeFrequency: "weekly" as const, priority: 0.8 })),
  ];
}
