...
export function ProductDetailClient({ product, similar, adminUrl }: { product: Product; similar: Product[]; adminUrl: string }) {
  ...
  const features = [
    ...
    `Official ${brand} product support`,
      ...
      <main className="marketplace-product-main">
        ...
        <section className="marketplace-product-layout">
          <div className="marketplace-gallery">
            <div className="marketplace-thumbnails">
              {[0, 1, 2, 3].map((index) => <button className={selectedImage === index ? "active" : ""} type="button" onClick={() => setSelectedImage(index)} key={index}><ProductArtwork product={product} /></button>)}
            </div>
            <div className={`marketplace-product-media media-angle-${selectedImage}`}><ProductArtwork product={product} priority /><span className="product-zoom-note">⌕ Hover to inspect</span>{product.compareAtCents && product.compareAtCents > product.priceCents ? <b className="market-detail-sale">Special price</b> : null}</div>
          </div>

          <section className="marketplace-product-copy">
            <div className="product-brand-line"><span>{brand}</span><a href="mailto:support@bjelectronics.shop?subject=Product%20enquiry">Ask about this product</a></div>
            <h1>{product.name}</h1>
            ...
            <div className="market-purchase-panel">
              <label>Quantity<div className="detail-quantity"><button type="button" onClick={() => setQuantity((value) => Math.max(1, value - 1))}>−</button><span>{quantity}</span><button type="button" onClick={() => setQuantity((value) => Math.min(Math.max(1, Math.min(20, product.inventoryQuantity)), value + 1))}>+</button></div></label>
              <button className="market-add-cart" type="button" disabled={adding || product.inventoryQuantity < 1} onClick={addToCart}>🛒 {adding ? "Adding…" : "Add to cart"}</button>
              <button className={`market-wishlist-button${saved ? " saved" : ""}`} type="button" onClick={toggleWishlist}>{saved ? "♥ Saved" : "♡ Wishlist"}</button>
            </div>
        ...
        <section className="market-product-info-tabs">
          <div className="info-tab-head"><button className="active" type="button">Description</button><button type="button">Specifications</button><button type="button">Delivery & warranty</button></div>
          <div className="info-tab-body"><div><span>BJ Electronics product overview</span><h2>Reliable technology for modern living.</h2><p>{product.description}</p><p>Product availability, price and publication status are controlled through the secure BJ Electronics administration platform. Final inventory is revalidated before an order is created.</p></div><dl><div><dt>Brand</dt><dd>{brand}</dd></div><div><dt>Category</dt><dd>{category}</dd></div><div><dt>Model / SKU</dt><dd>{product.sku}</dd></div><div><dt>Availability</dt><dd>{product.inventoryQuantity > 0 ? "In stock" : "Out of stock"}</dd></div><div><dt>Currency</dt><dd>{product.currency}</dd></div></dl></div>
        </section>
