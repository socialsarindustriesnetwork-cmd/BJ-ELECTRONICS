...
const categoryLabels: Record<string, string> = {
  ...
  monitor: "Laptops & Computing",
  earphones: "Audio & Headphones",
  headphones: "Audio & Headphones",
  speaker: "Audio & Headphones",
  watch: "Smart Watches & Wearables",
  accessory: "Power & Accessories",
...
export function CatalogListingClient({ products, adminUrl }: { products: Product[]; adminUrl: string }) {
  ...
  const categories = useMemo(() => {
    const inferred = new Set(products.map(productCategory));
    const ordered = marketplaceCategories.map((item) => item.label).filter((label) => inferred.has(label));
    const remaining = Array.from(inferred).filter((label) => !ordered.includes(label)).sort();
    return ["All products", ...ordered, ...remaining];
  }, [products]);
  ...
  return (
    <div className="store-shell listing-shell full-market-listing-shell marketplace-listing-shell">
      ...
      <main className="listing-main marketplace-listing-main">
        ...
        <section className="listing-discovery-banner">
          <div><span>BJ Electronics marketplace</span><h1>{category === "All products" ? "Find the right electronics and appliances" : category}</h1><p>Search live inventory, compare current prices and filter products by department, brand and availability.</p></div>
          <div className="listing-discovery-art" aria-hidden="true"><span>BJ</span><i /><b /></div>
        </section>

        <nav className="listing-category-chips" aria-label="Quick department filters">
          {categories.slice(0, 11).map((item) => <button key={item} className={category === item ? "active" : ""} type="button" onClick={() => setCategory(item)}>{item}</button>)}
        </nav>
        ...
        <div className="listing-layout marketplace-listing-layout">
          ...
          <section className="listing-results marketplace-listing-results">
            ...
            {filtered.length ? <div className={`listing-product-grid ${view}`}>{filtered.map((product) => <ProductCard product={product} key={product.id} />)}</div> : <div className="empty-state"><strong>No products match these filters.</strong><p>Try another department, price range or search term.</p><button type="button" onClick={clearFilters}>Reset filters</button></div>}
          </section>
        </div>
      </main>
      <StoreFooter />
