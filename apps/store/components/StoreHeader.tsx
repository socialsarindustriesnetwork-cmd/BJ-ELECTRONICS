      ...
      <header className="commerce-header caravan-header full-market-header">
        <div className="commerce-header-main">
          ...
          <form className="header-search caravan-search full-market-search" onSubmit={submitSearch} role="search">
            <select aria-label="Search department" value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)}>
        ...
        <nav className={`category-nav caravan-category-nav full-category-nav${menuOpen ? " open" : ""}`} aria-label="Product departments">
          <div className="category-nav-inner">
            <Link className="all-category-link" href="/categories" onClick={() => setMenuOpen(false)}>☰ Shop by department</Link>
            {marketplaceCategories.map((category) => <Link key={category.key} href={`/categories?category=${encodeURIComponent(category.label)}`} onClick={() => setMenuOpen(false)}>{category.short}</Link>)}
          </div>
        </nav>
      </header>
