...
export function StoreHeader({ adminUrl }: { adminUrl: string }) {
  ...
  return (
    <>
      <div className="utility-bar caravan-utility"><div className="utility-inner"><div className="utility-contact"><a href="mailto:support@bjelectronics.shop">✉ support@bjelectronics.shop</a><span>Sat–Thu · 9:00 AM–6:30 PM</span></div><div className="utility-links"><Link href="/about">About</Link><Link href="/contact">Contact</Link><Link href="/track-order">Track order</Link><a href={adminUrl}>Administration</a></div></div></div>
      <header className="commerce-header caravan-header">
        <div className="commerce-header-main">
          ...
          <form className="header-search caravan-search" onSubmit={submitSearch} role="search"><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search products, brands and categories…" aria-label="Search products" /><select aria-label="Search category" value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)}><option value="all">All categories</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></form>
          <nav className="commerce-actions" aria-label="Account actions"><Link href="/wishlist" className="header-action"><span aria-hidden="true">♡</span><small>Wishlist</small>{wishlistCount > 0 ? <b>{wishlistCount}</b> : null}</Link><Link href="/cart" className="header-action caravan-cart-action"><span aria-hidden="true">🛒</span><small>{cartCount} item{cartCount === 1 ? "" : "s"}</small><em>{formattedCartTotal}</em>{cartCount > 0 ? <b>{cartCount}</b> : null}</Link><a href={adminUrl} className="header-action"><span aria-hidden="true">♙</span><small>Profile</small></a></nav>
        </div>
        <nav className={`primary-store-nav${menuOpen ? " open" : ""}`} aria-label="Primary store navigation"><div className="primary-store-nav-inner"><Link href="/">Home</Link><Link href="/shop">Shop</Link><Link href="/about">About us</Link><Link href="/contact">Contact us</Link><Link href="/track-order">Track order</Link><Link className="primary-nav-deal" href="/shop?sort=discount">Special offers</Link></div></nav>
        <nav className={`category-nav caravan-category-nav${menuOpen ? " open" : ""}`} aria-label="Product categories"><div className="category-nav-inner"><Link className="all-category-link" href="/shop">☰ Browse all</Link>{categories.map((category) => <Link key={category} href={`/shop?category=${encodeURIComponent(category)}`}>{category}</Link>)}</div></nav>
      </header>
