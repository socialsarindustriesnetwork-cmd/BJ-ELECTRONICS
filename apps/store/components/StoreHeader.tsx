...
const deliveryAreas = ["Bangladesh", "Dhaka", "Chattogram", "Sylhet", "Rajshahi", "Khulna", "Rangpur", "Barishal"];
...
export function StoreHeader({ adminUrl }: { adminUrl: string }) {
  ...
  useEffect(() => {
    ...
    if (storedArea && deliveryAreas.includes(storedArea)) setDeliveryArea(storedArea);
  ...
  function changeDeliveryArea(value: string) {
    setDeliveryArea(value);
    localStorage.setItem("bje-delivery-area", value);
  }
    ...
    <>
      <div className="utility-bar marketplace-utility-bar">
        <div className="utility-inner marketplace-utility">
          <label className="delivery-selector">
            ...
            <select value={deliveryArea} onChange={(event) => changeDeliveryArea(event.target.value)} aria-label="Delivery area">
              {deliveryAreas.map((area) => <option key={area}>{area}</option>)}
            </select>
          </label>
          <div className="utility-promise"><span>✓ Authentic products</span><span>✓ Official warranty support</span></div>
      ...
      <header className="commerce-header marketplace-header">
        <div className="commerce-header-main">
          ...
          <nav className="commerce-actions marketplace-actions" aria-label="Account actions">
      ...
      <nav className="mobile-bottom-nav marketplace-mobile-nav" aria-label="Mobile navigation">
        <Link href="/"><span>⌂</span>Home</Link>
        <Link href="/categories"><span>▦</span>Browse</Link>
        <Link href="/wishlist"><span>♡</span>Saved{wishlistCount > 0 ? <b>{wishlistCount}</b> : null}</Link>
