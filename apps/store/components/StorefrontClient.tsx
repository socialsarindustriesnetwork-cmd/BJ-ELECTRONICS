    ...
    <div className="store-shell caravan-storefront">
      ...
      <main>
          ...
          <section className="retail-hero caravan-main-hero" aria-labelledby="caravan-hero-title">
            <div className="caravan-hero-copy">
              ...
              <div className="retail-hero-actions">
                <Link className="shop-primary" href="/shop">Shop all products</Link>
                <Link className="shop-secondary" href="/shop?sort=discount">View special offers</Link>
              </div>
            ...
            <div className="caravan-hero-product">
              ...
              <div className="hero-product-label"><small>Featured technology</small><strong>{heroProduct?.name ?? "Premium electronics collection"}</strong><Link href={heroProduct ? `/products/${heroProduct.slug}` : "/shop"}>View product →</Link></div>
            </div>
          </section>

          <aside className="hero-side-offers" aria-label="Featured store offers">
            <article className="hero-side-card side-card-blue"><span>Audio collection</span><h2>Clear sound, anywhere.</h2><Link href="/shop?category=Headphones">Shop audio</Link></article>
            <article className="hero-side-card side-card-red"><span>Mobile essentials</span><h2>Power your day.</h2><Link href="/shop?category=Power%20Banks">Shop power</Link></article>
          </aside>
        </section>
        ...
        <section className="retail-section category-section">
          <div className="retail-section-heading"><div><span>Browse faster</span><h2>Popular categories</h2></div><Link href="/shop">View all categories</Link></div>
        ...
        <section className="retail-section caravan-products-section demand-section">
          <div className="retail-section-heading"><div><span>Popular right now</span><h2>Top Demand</h2></div><div className="heading-actions"><span className={`catalog-live${live ? " connected" : ""}`}>{live ? "Live inventory" : "Connecting"}</span><Link href="/shop">View all</Link></div></div>
          {topDemand.length ? <div className="featured-product-grid caravan-product-grid">{topDemand.map((product) => <ProductCard compact product={product} key={product.id} />)}</div> : <div className="empty-state">Top-demand products will appear when the catalog is published.</div>}
        </section>

        <section className="retail-section caravan-products-section">
          <div className="retail-section-heading"><div><span>Recently added</span><h2>New arrivals</h2></div><div className="heading-actions"><span className={`catalog-live${live ? " connected" : ""}`}>{live ? "Live inventory" : "Connecting"}</span><Link href="/shop?sort=newest">View all</Link></div></div>
        ...
        <section className="wide-campaign-banner">
          <div><span>Performance week</span><h2>Build your complete setup.</h2><p>Pair laptops, monitors, audio and accessories from one trusted store.</p><Link href="/shop">Explore the collection</Link></div>
          <div className="campaign-device-art" aria-hidden="true"><i /><span /><b /></div>
        </section>

        <section className="retail-section caravan-products-section">
          <div className="retail-section-heading"><div><span>Customer favourites</span><h2>Featured products</h2></div><Link href="/shop">View all</Link></div>
        ...
        <section className="split-campaigns">
          <article className="split-campaign campaign-computing"><span>Computing essentials</span><h2>Work smarter with a complete setup.</h2><p>Discover laptops, monitors and accessories selected for productivity.</p><Link href="/shop?category=Laptops">Shop computing</Link></article>
          <article className="split-campaign campaign-lifestyle"><span>Connected lifestyle</span><h2>Audio and wearables for every day.</h2><p>Stay connected with dependable sound, smart watches and mobile power.</p><Link href="/shop?category=Smart%20Watches">Shop lifestyle tech</Link></article>
        </section>

        <section className="retail-section caravan-products-section">
          <div className="retail-section-heading"><div><span>Limited-time value</span><h2>Special offers</h2></div><Link href="/shop?sort=discount">Shop all deals</Link></div>
        ...
        <section className="brand-showcase">
          <div className="retail-section-heading"><div><span>Trusted names</span><h2>Shop leading brands</h2></div><Link href="/shop">Browse products</Link></div>
          <div className="brand-logo-grid">{brands.map((brand) => <Link href={`/shop?q=${encodeURIComponent(brand)}`} key={brand}>{brand}</Link>)}</div>
        </section>
