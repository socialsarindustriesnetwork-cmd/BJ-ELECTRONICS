...
const serviceHighlights = [["🚚", "Countrywide delivery", "Reliable shipping across Bangladesh"], ["✓", "Official warranty", "Clear product coverage and support"], ["↻", "Easy return help", "Transparent post-purchase assistance"], ["▣", "Secure checkout", "Protected cart and order processing"]];
...
function searchText(product: Product): string { return `${product.name} ${product.sku} ${product.description}`.toLowerCase(); }
...
export function StorefrontClient({ initialProducts, latestEventId, adminUrl }: { initialProducts: Product[]; latestEventId: number; adminUrl: string }) {
  ...
  useEffect(() => {
    ...
    source.addEventListener("commerce", (event) => {
      try { const payload = JSON.parse((event as MessageEvent<string>).data) as { id?: number }; if (typeof payload.id === "number") cursor.current = payload.id; } catch { /* Catalog refresh below remains authoritative. */ }
      void fetch("/api/catalog", { cache: "no-store" }).then((response) => response.json()).then((payload: { products?: Product[]; latestEventId?: number }) => { if (payload.products) setProducts(payload.products); if (typeof payload.latestEventId === "number") cursor.current = payload.latestEventId; }).catch(() => undefined);
    });
    return () => source.close();
  }, []);
  ...
  const topDemand = useMemo(() => [...products].sort((a, b) => { const aDiscount = a.compareAtCents ? a.compareAtCents - a.priceCents : 0; const bDiscount = b.compareAtCents ? b.compareAtCents - b.priceCents : 0; return bDiscount !== aDiscount ? bDiscount - aDiscount : b.inventoryQuantity - a.inventoryQuantity; }).slice(0, 8), [products]);
  const shelves = useMemo(() => shelfDefinitions.map((definition, index) => { const matched = products.filter((product) => definition.terms.some((term) => searchText(product).includes(term))); const fallback = products.slice(index * 4, index * 4 + 6); return { ...definition, products: (matched.length ? matched : fallback).slice(0, 6) }; }), [products]);
  const heroProduct = topDemand[0] ?? products[0];

  return <div className="store-shell caravan-storefront"><StoreHeader adminUrl={adminUrl} /><main>
    <section className="caravan-hero-shell"><aside className="hero-departments" aria-label="Shop departments"><h2>Shop by category</h2>{categories.map((category) => <Link key={category.name} href={`/shop?category=${encodeURIComponent(category.name)}`}><span>{category.icon}</span><div><strong>{category.name}</strong><small>{category.description}</small></div><b>›</b></Link>)}</aside><section className="retail-hero caravan-main-hero" aria-labelledby="caravan-hero-title"><div className="caravan-hero-copy"><p className="hero-kicker">Original products. Dependable service.</p><h1 id="caravan-hero-title">Technology for every part of your day.</h1><p>Explore trusted electronics, live inventory, secure ordering and responsive support—all in one modern BJ Electronics storefront.</p><div className="retail-hero-actions"><Link className="shop-primary" href="/shop">Shop all products</Link><Link className="shop-secondary" href="/shop?sort=discount">View special offers</Link></div><div className="hero-mini-proof"><span>✓ Live stock</span><span>✓ Secure checkout</span><span>✓ Fast support</span></div></div><div className="caravan-hero-product"><div className="hero-glow" /><div className="hero-laptop"><div className="hero-laptop-screen"><i /><i /><i /><span>BJ</span></div><div className="hero-laptop-base" /></div><div className="hero-product-label"><small>Featured technology</small><strong>{heroProduct?.name ?? "Premium electronics collection"}</strong><Link href={heroProduct ? `/products/${heroProduct.slug}` : "/shop"}>View product →</Link></div></div></section><aside className="hero-side-offers" aria-label="Featured store offers"><article className="hero-side-card side-card-blue"><span>Audio collection</span><h2>Clear sound, anywhere.</h2><Link href="/shop?category=Headphones">Shop audio</Link></article><article className="hero-side-card side-card-red"><span>Mobile essentials</span><h2>Power your day.</h2><Link href="/shop?category=Power%20Banks">Shop power</Link></article></aside></section>
